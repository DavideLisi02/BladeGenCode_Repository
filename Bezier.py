    ...
    def __init__(self, curve_parameters, **kwargs):
        ...
        if self.object == 'Blade' and self.definition == 'beta-M%':
            ...
            if self.spline_degree == 2:
                    ...
                    self.x_hub_values = np.linspace(0, 1, 100)
                    ...
                    self.x_shr_values = np.linspace(0, 1, 100)
                    ...
                    Lead_Hub_point = [0.005,self.r2h]  # Point for Leading Edge at hub | 0.005 stands for (distance between leading edge at hub and inducer)/(L_comp)
                    points_hub = np.insert(points_hub, 1, Lead_Hub_point, axis=0)  # Insert Lead_Hub_point as second point
                    Trail_Hub_point = [1, self.r4*0.999]  # Point for Trailing Edge at hub
                    ...
                    Lead_Shr_point = [0.008,self.r2s]  # Point for Leading Edge at hub | 0.008 stands for (distance between leading edge at shroud and inducer)/(L_comp-b4)
                    points_shr = np.insert(points_shr, 1, Lead_Shr_point, axis=0)  # Insert Lead_Hub_point as second point
                    Trail_Shr_point = [1, self.r4*0.999]  # Point for Trailing Edge at hub
