...
pars_list = [ParametrizationSettings(
                ...
                HubShroud_1D_dimensions = {'object':'HubShroud','HubShr_bezier_N':HubShr_bezier_N,'definition':'xz','spline_degree':2,'L_ind':L_ind,'L_comp':L_comp,'r2s':r2s,'r2h':r2h,'r4':r4,'b4':b4,'r5':r5,'w1_hb':w1_i,'w1_sh':w1_i},
                ...
                index = i,
                par_name = f"_{str(w1_i).replace('.', '')}")
                for i,w1_i in enumerate(w1_hbsh)]
...
# Display the figure
fig.show()
print(f"\nExpected time to create BladeGen files: {3.5*len(w1_hbsh)/60} min\n")
...
if Go_on in ["y","Y"]:
    ...
    results_dict = { i: {"w1_hbsh": w1_i,"Results":None} #The "Results" string at the end is where the results will be stored (Still to be implemented), while the first one is the index
                    for i,w1_i in enumerate(w1_hbsh)}
